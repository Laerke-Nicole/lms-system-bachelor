...
@section('content')
    ...
    <x-blocks.form action="{{ route('ab_inventech.update', $abInventech->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <x-elements.textarea col="col-12 col-lg-4" label="Email" name="ab_inventech_mail" value="{{ $abInventech->ab_inventech_mail }}"/>
        <x-elements.input col="col-12 col-lg-4" label="Phone" name="ab_inventech_phone" value="{{ $abInventech->ab_inventech_phone }}"/>
        <x-elements.input col="col-12 col-lg-4" label="Logo" name="logo" type="file" value="{{ $abInventech->logo }}"/>
