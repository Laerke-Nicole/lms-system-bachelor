...
<div class="certificate-container">
    ...
    @if($trainingUser && $trainingUser->user)
        <h2 class="certificate__recipient">
            {{ $trainingUser->user->first_name }} {{ $trainingUser->user->last_name }}
        </h2>
    @endif

    @if($trainingUser && $trainingUser->training && $trainingUser->training->course && $trainingUser->training->trainingSlot && $trainingUser->training->trainingSlot->training_date)
        <p class="certificate__training">
            for completing the training
            {{ $trainingUser->training->course->title }}
            on {{ $trainingUser->training->trainingSlot->training_date->format('d M Y') }}.
        </p>
    @endif
    ...
    @if($trainingUser)
        <div class="certificate__signature-block">
            @if($trainingUser->temporary_signature)
                <img src="{{ uploads_url($trainingUser->temporary_signature) }}" class="certificate__signature-image">
            @endif
    ...
    @if($trainingUser && $trainingUser->valid_until)
        <p class="certificate__valid-until">Certificate is valid until {{ $trainingUser->valid_until->format('d M Y') }}.</p>
    @endif
