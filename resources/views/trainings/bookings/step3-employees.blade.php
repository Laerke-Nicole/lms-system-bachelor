...
@section('content')
    ...
    <h3 class="mb-5">Who's joining?</h3>

    <div class="row g-3">
        @foreach($employees as $employee)
            <div class="col-12 col-md-6 col-lg-3">
                <x-blocks.form action="{{ route('trainings.bookings.slot.store') }}" method="POST" class="mb-0 w-100 d-flex flex-column">
                    <input type="hidden" name="user_ids" value="{{ $employee->id }}">

                    <button type="submit" class="w-100 border-0 bg-white rounded-3 shadow-sm text-start d-flex flex-column p-0">
